...
Page({
  ...
  onLoad: function () {
    ...
    if (wx.getStorageSync('checkIn') && wx.getStorageSync('checkOut')) {
      ...
      this.data.startDate = wx.getStorageSync('checkIn');
      this.data.endDate = wx.getStorageSync('checkOut');
    }
    var date = new Date();
    //获取当前的年月
  ...
  selectAction: function (e) {
    ...
    if (time < 0 || time >= 30) {
      ...
      return;
    }else if(time == 29 && this.data.endDate) {
      // console.log('haha');
      // console.log(selectDate);
      var std = selectDate.getTime();
      // var endd = new Date(parseInt(std+86400000)).toLocaleString().substr(0,9);
      var endd = new Date(parseInt(std+86400000));
      // console.log(Data.formatDate(endd, "yyyy-MM-dd"));
      wx.setStorageSync('checkIn', Data.formatDate(selectDate, "yyyy-MM-dd"));
      wx.setStorageSync('checkOut', Data.formatDate(endd, "yyyy-MM-dd"));
      wx.navigateBack({
        delta: 1
      })
      return;
    ...
    if (this.data.startDate && this.data.endDate) {
      ...
      var sd = new Date(this.data.startDate);
      var ed = new Date(Data.formatDate(selectDate, "yyyy-MM-dd"));
      var oneMs = 86400000;
      if ((ed - sd) > oneMs*7 || (ed - sd) == 0) {
        wx.showToast({
          title: '最少选择1天，最多选择7天',
          icon: 'none',
          duration: 2000
    ...
    if (this.data.startDate && this.data.endDate) {
      ...
      wx.setStorageSync('checkIn', sDate);
      wx.setStorageSync('checkOut', eDate);
      wx.navigateBack({
